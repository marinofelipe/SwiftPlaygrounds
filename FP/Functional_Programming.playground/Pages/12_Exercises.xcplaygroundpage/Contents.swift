...
struct PredicateSet<A> {
  ...
  func contramap<B>(_ f: @escaping (B) -> A) -> PredicateSet<B> {
    return PredicateSet<B>(run: f >>> run)
  }
}
...
let isPowerOf2 = PredicateSet(run: isPowerOfTwo)
...
let isPowerOf2Minus1 = PredicateSet { isPowerOf2.run($0 - 1) }

isPowerOf2Minus1.run(1) // 0
isPowerOf2Minus1.run(2) // 1
isPowerOf2Minus1.run(3) // 2
isPowerOf2Minus1.run(4) // 3
isPowerOf2Minus1.run(5) // 4
isPowerOf2Minus1.run(6) // 5
isPowerOf2Minus1.run(7) // 6

/*:
 5c.) Find an algorithm online for testing if an integer is prime, and turn it into a predicate `isPrime: PredicateSet<Int>`.
 */

func isPrimeFunc(_ number: Int) -> Bool {
    guard number > 1 else { return false }

    return (2..<number).contains { number % $0 == 0 } == false
}

let isPrime = PredicateSet(run: isPrimeFunc)
